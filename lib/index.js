...
app.post('/webhook/push', jsonParser, (req, res) => {
  ...
  const modifiedFiles = uniq(flatMap(commit => union(commit.modified, commit.added), filteredCommits))
  log('modifiedFiles: ' + modifiedFiles)

  const syncDestinations = flatten(
    modifiedFiles
      .map(file => getFileDestinations(branchName, repository.name, owner.name, file))
